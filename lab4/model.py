...
class seq2seqAtt(nn.Module):
    ...
    def forward(self,target_h,source_hs):
        ...
        #out = torch.cat((target_h_rep,source_hs),dim=2)
        concat_output = torch.tanh(self.ff_concat(torch.cat((target_h_rep,source_hs),dim=-1)))
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        out = self.embedding(input)
        (r,h)= self.rnn(out,h)
        h_tilde = self.ff_concat(torch.cat((source_context,h),dim=-1))
        h_tilde = torch.tanh(h_tilde)
        #out = torch.cat((source_context,h),dim=0)
...
class seq2seqModel(nn.Module):
    ...
    def forward(self,input,max_size,is_prod):
        ...
        while True:
            ...
            target_input = torch.argmax(prediction,2)
    ...
    def predict(self,source_nl):
        ...
        logits, norm_scores = self.forward(source_ints,self.max_size,True) # (seq) -> (<=max_size,vocab)
        target_ints = logits.argmax(-1).squeeze() # (<=max_size,1) -> (<=max_size)
        target_nl = self.targetInts_to_nl(target_ints.tolist())
        return ' '.join(target_nl)

    def predict_att(self,source_nl):
        source_ints = self.sourceNl_to_ints(source_nl)
        logits, _ = self.forward(source_ints,self.max_size,True)
        logits = torch.softmax(logits,dim=-1)
        indices = torch.max(logits,dim=-1)[1]
        indices = indices.squeeze()
        targets_nl =  self.targetInts_to_nl(indices.tolist())
        weights = logits[:len(source_nl.split()),indices.tolist()].detach()
        target_nl = self.targetInts_to_nl(indices.tolist())
        
        return target_nl, source_nl,weights
        #print(type(target_nl))
