...
def sample_examples(docs,max_window_size,n_windows):
    ...
    for i,doc in enumerate(docs):
        ...
        #window_size=np.random.randint(1,max_window_size,n_windows)
    ...
    all_negs = list(np.random.choice(list(vocab_inv.keys()),size=n_negs*len(windows),p=neg_distr,replace=True))### fill the gap (sample n_negs*len(windows) negatives according to some probability distribution ###
...
def compute_gradients(pos,negs,target,prodpos,prodnegs):
    ...
    ### fill the gaps ### 
    #partials_pos= [- Wt[target,]*factors_pos[p] for p in range(len(pos))]
    #partials_negs= [Wt[target,]*factors_negs[n] for n in range(len(negs))]
    ...
    #term_pos= [- Wc[p,]*factors_pos[p] for p in range(len(pos))]
    #term_negs= [ Wc[n,]*factors_negs[n] for n in range(len(negs))]
...
path_read = "/home/khaoula/Documents/MVA/ALTEGRAD/TP2/for_moodle/data/"
...
if train:
    ...
    np.save(path_write + 'input_vecs1',Wt,allow_pickle=False) # pickle disabled for portability reasons
    np.save(path_write + 'output_vecs1',Wc,allow_pickle=False)
...
if not train:
    ...
    my_cos_similarity("king","man")
    my_cos_similarity("movie","film")
    my_cos_similarity("banana","sky")
    my_cos_similarity("movie","banana")
    my_cos_similarity("king","queen")
    my_cos_similarity("bad","horrible")
    ...
    fig.savefig(path_write + '3.pdf',dpi=300)
    fig.show()
