...
path_to_abstracts = '/home/khaoula/Documents/MVA/ALTEGRAD/for_moodle/data/Hulth2003testing/abstracts' ### fill me! ###
...
path_to_keywords = '/home/khaoula/Documents/for_moodle/data/Hulth2003testing/uncontr/' ### fill me! ###
...
gs = [terms_to_graph(abstract,w) for abstract in abstracts_cleaned]
...
for counter,g in enumerate(gs):
    ...
    core_numbers_s = sorted(core_numbers.items(), key=operator.itemgetter(1), reverse=True)
    
    max_c_n = max(core_numbers.values())
    key = [kwd for kwd, c_n in core_numbers.items() if c_n == max_c_n]
    keywords['kc'].append(key)
...
for counter,vector in enumerate(vectors_list):
 ...
 ### fill the gap (keep only non zero values, i.e., the words in the document. store the results in a list named 'nonzero') ###
    nonzero = [my_tuple for my_tuple in terms_weights if my_tuple[1] != 0]
